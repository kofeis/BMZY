  ...
  return {
    ...
    execute: function () {
      ...
      _export("Tool_NetWork", Tool_NetWork = (_dec = ccclass('Tool_NetWork'), _dec(_class = (_class2 = class Tool_NetWork extends Component {
         ...
         */
        async HttpRequest(Options, Cache = "no-cache") {
          try {
            // 处理查询参数
        ...
        async Request_Delete(Url, Params) {
          //如果没有Params参数，执行单个删除
          ...
          try {
            ...
            const DeleteResults = await Promise.all(ItemToDelete.map(Item => this.HttpRequest({
              Url: `${Url}/${Item.id}`,
              Method: "DELETE"
            }).then(() => ({
              Id: Item.id,
        ...
        async Request_Modify(Url, Data, Params, Method = "PUT") {
          //如果没有Params参数，执行单个更新
          if (!Params || Object.keys(Params).length === 0) {
            return await this.HttpRequest({
              Url: Url,
          ...
          try {
            ...
            const UpdateResults = await Promise.all(ItemToUpdate.map(Item => this.HttpRequest({
              Url: `${Url}/${Item.id}`,
              Method: Method,
              Data: Data
            }).then(Result => ({
        ...
        */
        ...
        async Request_Check(Url, Params) {
          return await this.HttpRequest({
            Url: Url,
            Method: "GET",
            Params: Params
...
//# sourceMappingURL=4122be4fb207a48faab2ecbb174c4a9683eb37bb.js.map