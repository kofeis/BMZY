  ...
  return {
    ...
    execute: function () {
      ...
      _export("Resoure_Data", Resoure_Data = (_dec = ccclass('Resoure_Data'), _dec2 = property({
        ...
        async Update_Data_All(Url, Params, UpdateData) {
          try {
            // 1. 构建查询字符串并获取待更新数据
            ...
            switch (Status) {
              case 200:
                ...
                const UpdateResults = await Promise.all(ItemsToUpdate.map(item => this.Update_Data(`${Url}/${item.id}`, UpdateData)));
        ...
        async Delete_Data_All(Url, Params) {
          try {
            // 查询符合条件的数据
            const QueryString = new URLSearchParams(Params).toString();
            const Response = await fetch(`${Url}?${QueryString}`, {
            ...
            switch (Status) {
              case 200:
                ...
                const DeleteResults = await Promise.all(ItemsToDelete.map(item => this.Delete_Data(`${Url}/${item.id}`).then(() => ({
                  id: item.id,
                  success: true
                })).catch(error => {
        ...
        */
        ...
        async Load_Scene(Path, Progress_Mask, Progress_Bar, Value_Label, Mode = ScreenMode.Horizontal) {
...
//# sourceMappingURL=58aa523da28252217ff09c8685beee7c9b30bca7.js.map