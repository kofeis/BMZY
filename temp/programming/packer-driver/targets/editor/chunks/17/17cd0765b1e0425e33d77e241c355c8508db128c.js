  ...
  return {
    ...
    execute: function () {
      ...
      _export("Tool_Animation", Tool_Animation = (_dec = ccclass('Tool_Animation'), _dec(_class = (_class2 = class Tool_Animation extends Component {
        ...
        Timing(Obj_Timing, Time = 60, Call = () => {}) {
          Obj_Timing.num = 0;
        ...
        TimReset(Obj_Timing, Time = 60, Call = () => {}) {
          console.debug("倒计时重置");
        ...
        TimPause(Obj_Timing, Time = 60, Call = () => {}) {
          console.debug("倒计时暂停");
        ...
        TimResume(Obj_Timing, Time = 60, Call = () => {}) {
          console.debug("倒计时暂停");
        ...
        TimStop(Obj_Timing, Time = 60, Call = () => {}) {
          console.debug("倒计时停止");
          this.Tween_Stop_Target(Obj_Timing);
        }
        /**暂停缓动对象 */
        ...
        Animation_Num_Jump_Time(Component_Label, Value_Start, Value_Change, Time, Obj_Start, Id, Call = () => {}, Lenth = 2, Easing = "linear") {
          const Obj_End = {
            num: 1
          };
        ...
        Animation_Num_Jump_Speed(Component_Label, Value_Start, Value_Change, Speed, Obj_Start, Id, Call = () => {}, Length = 2, Easing = "linear") {
          // 计算动画时间（时间 = 变化量 / 速度）
        ...
        */
        ...
        Get_Distance_Limited(Pos_Start, Pos_Finish, Dis) {
          // 计算两点之间的距离
          const Distance = this.Get_Distance(Pos_Start.clone(), Pos_Finish.clone()); // 如果两点之间的距离小于或等于目标距离,则返回目标点坐标
          ...
          const Direction = Pos_Finish.clone().subtract(Pos_Start.clone()).normalize(); // 计算中间位置坐标
        ...
        Position_Restricted(Target_Position, Min_Position, Max_Position, Ortho_Height_Multiple = 1) {
          /** 设计分辨率 */
          const Design_Width = (_crd && Tool_UI === void 0 ? (_reportPossibleCrUseOfTool_UI({
            error: Error()
          }), Tool_UI) : Tool_UI).instance.Get_Width_Design() * Ortho_Height_Multiple;
          const Design_Height = (_crd && Tool_UI === void 0 ? (_reportPossibleCrUseOfTool_UI({
            error: Error()
          ...
          const clamp = (value, min, max) => Math.max(min, Math.min(value, max)); // 限制目标坐标
        ...
        */
        ...
        Animation_Node_Move_Speed(Node_Set, Position, Speed = 1000, Call = () => {}, Easing = "linear") {
          if (Speed <= 0) {
            return;
          }

          let Move_Time = this.Get_Move_Time(Speed, Node_Set.position, Position);
        ...
        */
        ...
        async Animation_Node_Move_Time(Node_Set, Position, Time = 1, Call = () => {}, Easing = "linear") {
          return new Promise((resolve, reject) => {
            if (Time < 0) {
              return;
            } // this.Tween_Stop_Target(Node_Set);
        ...
        Animation_Nodes_Swap_Pos_Speed(Node_A, Node_B, Speed = 1000, Call = () => {}, Easing = "linear") {
          if (Speed <= 0) {
            return;
          }
        ...
        Animation_Nodes_Move_Swap_Time(Node_A, Node_B, Time = 1, Call = () => {}, Easing = "linear") {
          if (Time < 0) {
            return;
          }
        ...
        */
        ...
        Animation_Node_Move_Bezier_Time(Node_Set, Target_Pos, Id, PointA_Pos, PointB_Pos, Time = 1, Call = () => {}) {
          if (Time < 0) {
            return;
          } // 定义贝塞尔曲线的控制点
          ...
          const End_Position = Target_Pos.clone(); // 创建贝塞尔曲线函数
        ...
        Animation_Node_Move_Bezier_Speed(Node_Set, Target_Pos, Id, PointA_Pos, PointB_Pos, Speed = 520, Call = () => {}) {
          // 获取当前节点位置
          const Pos_Start = Node_Set.position.clone();
        ...
        */
        ...
        Animation_Node_Move_Bezier_Time1(Node_Set, Target_Pos, Obj_Start, Id, PointA_Pos, PointB_Pos, Time = 1, Call = () => {}) {
          if (Time < 0) {
            return;
          } // 定义贝塞尔曲线的控制点
        ...
        Animation_Node_Move_Parabola_Time(Node_Set, Target_Pos, Time = 1, Obj_Start, Id, Call = () => {}) {
          if (Time < 0) {
            return;
          } // 定义起始坐标和目标坐标
          ...
          const Pos_Start = Node_Set.position.clone();
          const End_Position = Target_Pos.clone(); // 计算控制点，形成抛物线

          const Control_Point = new Vec3((Pos_Start.x + End_Position.x) / 2, Math.max(Pos_Start.y, End_Position.y) + Math.abs(Target_Pos.y - Pos_Start.y) * 0.5, // 向上偏移
          0); // 创建贝塞尔曲线函数
        ...
        Animation_Nodes_Move_Speed(Node_Set, Position, Speed = 1000, Call = () => {}) {
          if (Speed <= 0) {
            return;
          }
        ...
        Animation_Nodes_Move_Time(Node_Set, Position, Time = 1, Call = () => {}) {
        ...
        Animation_Node_Move_Follow_Speed(Node_Set, Target_Node, Speed = 1000, Call = () => {}) {
        ...
        Animation_Hit(Node_Set, Hit_time, Call = () => {}, Color = color(255, 255, 255, 255)) {
          ...
          let Original_Color = color(Component_Sprite.color.a, Component_Sprite.color.g, Component_Sprite.color.b, Component_Sprite.color.a);
        ...
        Animation_Hit_Opacity(Node_Set, Hit_time, Call = () => {}, Min_Opacity = 155, Max_Opacity = 255) {
        ...
        */
        ...
        Animation_Death_Effect(Node_Set, Duration, Call = () => {}, Min_Scale = 0, Max_Scale = 1.5, Min_Opacity = 0, Max_Opacity = 255) {
          // 获取或添加 UIOpacity 组件
          let Component_UIOpacity = Node_Set.getComponent(UIOpacity);
          ...
          const Initial_Opacity = Max_Opacity; // 设置初始透明度
        ...
        Anim_Show_Node(Node_Set, Time = 0.5, Call = () => {}) {
        ...
        Anim_Hide_Node(Node_Set, Time = 0.5, Call = () => {}) {
          let Component = Node_Set.getComponent(UIOpacity);
        ...
        Anim_Show_Window(Node_Set, Time = 0.5, Call = () => {}) {
          let Scale_Current = v3(Node_Set.scale.x, Node_Set.scale.y, Node_Set.scale.z);
        ...
        Anim_Frame_Play(Anim, Name = null, Speed = 1, Call = () => {}) {
          if (Anim == null) {
            console.error("动画缺失");
            return;
          } // if (Name == null || Name == "") {
        ...
        Anim_Frame_Play_Plus(Anim, Name = null, Speed = 1, Min_Range = null, Max_Range = null, WrapMode = null, RepeatCount = null, Delay = 0, Call = () => {}) {
          if (Anim == null) {
            console.error("动画缺失");
            return;
          }
        ...
        Anim_Frame_CrossFade(Anim, Name = null, Duration = 0, Call = () => {}) {
          if (Anim == null) {
            console.error("动画缺失");
            return;
          }
        ...
        Anim_Frame_Resume(Anim, Name = null, Speed = 1, Call = () => {}) {
          if (Anim == null) {
            console.error("动画缺失");
            return;
          }
        ...
        Anim_Frame_Resume_Plus(Anim, Name = null, Speed = 1, Min_Range = null, Max_Range = null, WrapMode = null, RepeatCount = null, Delay = 0, Call = () => {}) {
          if (Anim == null) {
            console.error("动画缺失");
            return;
          }
          ...
          let State = Anim.getState(Name); // console.log("是否已被暂停或停止:", State.isMotionless);
        ...
        Anim_Frame_Speed(Anim, Name = null, Speed = 1) {
          if (Anim == null) {
            console.error("动画缺失");
            return;
          }
        ...
        Anim_On(Node_Set, Call = () => {}) {
          let Component_UIOpacity = (_crd && Tool_UI === void 0 ? (_reportPossibleCrUseOfTool_UI({
            error: Error()
        ...
        async Anim_Dissolve(Node_Set, Set_Material, Time = 1, Call = () => {}) {
          if (!(_crd && Tool_Other === void 0 ? (_reportPossibleCrUseOfTool_Other({
            error: Error()
          ...
          return new Promise(res => {
            const Sp = Node_Set.getComponent(Sprite); // 确保为每个节点创建独立的材质实例
        ...
        Get_Min_Distance_Pos_Arr(Position_World, Target_Position_World_Arr) {
          ...
          let Min_Index = 0;
          let Min_Distance = Number.MAX_VALUE; // 遍历数组，找到距离最小的点
        ...
        Get_Min_Distance_Node(Position_World, Target_Nodes) {
          ...
          let Min_Index = -1;
          let Min_Distance = Number.MAX_VALUE; // 遍历查找最近节点

          for (let i = 0; i < NodesToCheck.length; i++) {
            const Node = NodesToCheck[i]; // 检查节点有效性

            if (!(Node != null && Node.isValid) || !Node.worldPosition) continue; // 计算距离

            const Child_Position = NodesToCheck[i].worldPosition;
            const Distance = this.Get_Distance(Position_World, Child_Position); // 更新最近节点
...
//# sourceMappingURL=17cd0765b1e0425e33d77e241c355c8508db128c.js.map