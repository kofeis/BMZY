    ...
    execute: function () {
      ...
      _export("Tool_Event", Tool_Event = (_dec = ccclass('Tool_Event'), _dec(_class2 = (_class3 = class Tool_Event extends Component {
        ...
        */
        ...
        Event_Dispatch(Event_Name, Args = null) {
          let list = this.Events[Event_Name];
        ...
        On_Event_TOUCH_START(Node_Set, Call = Event => {}) {
          Node_Set.on(NodeEventType.TOUCH_START, Call, Node_Set);
        }
        /**注册触摸结束监听 */


        On_Event_TOUCH_END(Node_Set, Call = Event => {}, Target, UseCapture) {
          // 如果传入了 Target 则使用它，否则默认用 Node_Set 作为 this 上下文
          const TargetObj = Target !== undefined ? Target : Node_Set; // 如果传入了 UseCapture 则使用它，否则默认 false

          const IsUseCapture = UseCapture !== undefined ? UseCapture : false; // 注册事件监听
        ...
        On_Event_TOUCH_Double(Node_Set, Touch_Count_All = 2, Touch_Interval = 0.25, Double_On_Call = () => {}, Double_Off_Call = () => {}) {
          let Start_Obj = {
            num: 0
          };
          let Touch_Count = 0;
        ...
        On_Event_TOUCH_Drag(Node_Set, Interactable = () => true, TOUCH_START_Call = () => {}, TOUCH_MOVE_Call = () => {}, TOUCH_END_Call = () => {}) {
          // 触摸改变位置
          let Pos_Touch_Change = null; // 触摸开始位置

          let Pos_Touch_Start = null; // 触摸结束位置

          let Pos_Touch_End = null; // 设计分辨率

          const HalfScreenWidth = view.getDesignResolutionSize().width / 2;
          const HalfScreenHeight = view.getDesignResolutionSize().height / 2; // 工具函数：计算触摸位置相对于屏幕中心的位置

          const Get_Touch_Position = Event => {
          ...
          this.On_Event_TOUCH_START(Node_Set, Event => {
            ...
            TOUCH_START_Call == null || TOUCH_START_Call(Event, Pos_Touch_Start);
          });
          this.On_Event_TOUCH_MOVE(Node_Set, Event => {
            // 如果不可交互或没有起始位置，直接返回
            ...
            const Pos_Target = Node_Set.position.clone().add(Pos_Touch.clone().subtract(Pos_Touch_Change)); // 更新节点位置

            Node_Set.position = Pos_Target; // 更新起始触摸位置

            Pos_Touch_Change = Pos_Touch;
            TOUCH_MOVE_Call == null || TOUCH_MOVE_Call(Event, Pos_Touch_Start);
          });
          this.On_Event_TOUCH_END(Node_Set, Event => {
            ...
            TOUCH_END_Call == null || TOUCH_END_Call(Event, Pos_Touch_Start, Pos_Touch_End);
        ...
        Set_Btn_Event_Add(Node_Set, Target_Node, Scr_Name, Fun_Name, CustomEventData = null) {
        ...
        Set_Btn_Event_Anim(Node_Set, Anim_Switch = false) {
          let Component_Button = Node_Set.getComponent(Button);
        ...
        */
        ...
        Draw_Line_RigidBody(Canvas_Node, Stroke_Color = color(100, 255, 133), Line_Width = 10) {
          macro.ENABLE_MULTI_TOUCH = false;
          let CurGraphics = null;
          let Point_List = []; //前一个斜率

          let PreK = 0;

          const Get_UI_Pos = Pos => {
            var _CurGraphics;
          ...
          const Should_Add_Point = (DiffX, DiffY) => {
            //斜率变化的容忍度
            const K = 0.001;
        ...
        Draw_Line(Canvas_Node, Arr_Pos, Stroke_Color = color(100, 255, 133), Line_Width = 10) {
...
//# sourceMappingURL=c547803a5441b77715da97b4ffa444a73d2739fe.js.map