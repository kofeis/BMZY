System.register(["cc"], function (_export, _context) {
  ...
  var _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, _decorator, Component, find, isValid, sys, UITransform, v3, Vec3, view, Widget, instantiate, Sprite, SpriteFrame, color, UIOpacity, Label, Rect, _dec, _class, _class2, _crd, ccclass, property, Tool_UI;
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "961fad3PAdHYqvaHcikQfd6", "Tool_NextWork", undefined);
      ...
      _export("Tool_UI", Tool_UI = (_dec = ccclass('Tool_UI'), _dec(_class = (_class2 = class Tool_UI extends Component {
        /**获取组件 */
        Component_Get(Node_Set, Class_Constructor) {
          if (!Node_Set || !isValid(Node_Set, true)) {
            return null;
          }

          var Component = Node_Set.getComponent(Class_Constructor);
        ...
        Component_Check(Node_Set, Class_Constructor) {
          if (!Node_Set || !isValid(Node_Set, true)) {
            return false;
          }
        ...
        Component_On(Node_Set, Class_Constructor) {
          var Component = this.Component_Get(Node_Set, Class_Constructor);
          Component.enabled = true;
        }
        /**禁用组件 */


        Component_Off(Node_Set, Class_Constructor) {
          var Component = this.Component_Get(Node_Set, Class_Constructor);
        ...
        */
        ...
        isMobile() {
          return [sys.Platform.MOBILE_BROWSER, sys.Platform.ANDROID, sys.Platform.IOS, sys.Platform.OHOS, sys.Platform.OPENHARMONY].indexOf(sys.platform) !== -1;
        }
        /**
         * 检查是否是原生平台
         */
        ...
        isNative() {
          return [sys.Platform.ANDROID, sys.Platform.IOS, sys.Platform.WIN32, sys.Platform.MACOS, sys.Platform.OHOS, sys.Platform.OPENHARMONY].indexOf(sys.platform) !== -1;
        }
        /**获取 设计 高度 */
        ...
        Get_Node_Is_Valid(Node_Set, Strict_Mode) {
        ...
        Get_Touch_Position(Event) {
          // 设计分辨率
          var HalfScreenWidth = this.Get_Width_Design() / 2;
          var HalfScreenHeight = this.Get_Height_Design() / 2; // 工具函数：计算触摸位置相对于屏幕中心的位置
        ...
        Set_SpriteFrame(Node_Set, Sprite_Frame) {
          var Component_Sprite = this.Component_Get(Node_Set, Sprite);
          Component_Sprite.spriteFrame = Sprite_Frame;
        }
        /**设置精灵颜色 */


        Set_Color_Sprite(Node_Set, Sprite_Color) {
          if (Sprite_Color === void 0) {
        ...
        Set_Color_Label(Node_Set, Sprite_Color) {
          if (Sprite_Color === void 0) {
            Sprite_Color = color(255, 255, 255, 255);
          }

          var Component_Label = this.Component_Get(Node_Set, Label);
          Component_Label.color = Sprite_Color;
        }
        /**设置不透明度 */


        Set_UIOpacity(Node_Set, Opacity) {
          if (Opacity === void 0) {
            Opacity = 255;
          }

          var Component_UIOpacity = this.Component_Get(Node_Set, UIOpacity);
          Component_UIOpacity.opacity = Opacity;
        }
        /**设置节点坐标 */


        Set_Pos(Node_Set, Pos) {
        ...
        Set_Pos_Word_Same(Node_Set, Target_Node) {
        ...
        Get_Target_Node_Local_Pos(Node_Set, Target_Node) {
          // 获取 Target_Node 的世界坐标
          var Target_World_Pos = Target_Node.worldPosition; // 将世界坐标转换为 Node_Set 的局部坐标

          var Local_Pos = Node_Set.parent ? Node_Set.parent.getComponent(UITransform).convertToNodeSpaceAR(Target_World_Pos) : Target_World_Pos;
          return Local_Pos;
        }
        /**获取分割后的精灵数组 */


        Get_Split_SpriteFrame_Array(spriteFrame, rows, cols) {
          var spriteFrames = [];
          var tex = spriteFrame.texture;
          var texWidth = tex.width;
          var texHeight = tex.height;
          var frameWidth = texWidth / cols;
          var frameHeight = texHeight / rows;
        ...
        Get_Pos_Is_Equal(Pos, Pos_Target, precision) {
          if (precision === void 0) {
            precision = 0.0001;
          }

          return this.FloatIs_Equal(Pos.x, Pos_Target.x, precision) && this.FloatIs_Equal(Pos.y, Pos_Target.y, precision) && this.FloatIs_Equal(Pos.z, Pos_Target.z, precision);
        }
        /**
         * 判断两个浮点数是否在精度范围内相等
         * @param a 第一个数
        ...
        Node_Index_Set(Node_Set, Index) {
          var Lenth = Node_Set.parent.children.length - 1;
        ...
        Node_Index_Swap(Node_1, Node_2) {
          ...
          var Index_1 = Node_1.getSiblingIndex();
          var Index_2 = Node_2.getSiblingIndex(); // 索引相同，无需交换
        ...
        Arr_Increase(Arrays, Element) {
          Arrays.push(Element);
        }
        /** 数组 元素 批量增加 */


        Arr_Increase_Multiple(Arrays, Elements) {
          Arrays.push(...Elements);
        }
        /** 数组 元素 删除 */


        Arr_Delete(Arrays, Element) {
          var Index = this.Arr_Check_Index(Arrays, Element);
        ...
        Arr_Revise(Arrays, Element_Old, Element_New) {
          var Index = this.Arr_Check_Index(Arrays, Element_Old);
        ...
        Arr_Check_Index(Arrays, Element) {
          return Arrays.indexOf(Element);
        }
        /** 数组 元素 过滤 */


        Arr_Filter(Arrays, predicate) {
          return Arrays.filter(predicate);
        }
        /** 数组 元素 合并 */


        Arr_Merge(Arrays_A, Arrays_B) {
          return [...Arrays_A, ...Arrays_B];
        }
        /** 数组 元素 交换位置 */


        Arr_Swap(Arrays, Element_A, Element_B) {
          var Index_A = this.Arr_Check_Index(Arrays, Element_A);
          var Index_B = this.Arr_Check_Index(Arrays, Element_B);
        ...
        Hide_Node_Children_Except(Node_Set, Except_Node) {
          Node_Set.children.forEach(element => {
            for (var i = 0; i < Except_Node.length; i++) {
        ...
        Set_Prefab_Node(Prefab, Parent_Node, Position, Name, Call) {
          if (Call === void 0) {
            Call = () => {};
          }

          var Node_Prefab = instantiate(Prefab);
          Node_Prefab.parent = Parent_Node; // 如果 Name 有值，就赋值，否则不做任何操作
        ...
        Set_Pos_Word(Set_Pos, Target_Node) {
          Target_Node.setWorldPosition(Set_Pos);
        }
        /**获取世界坐标 */


        Get_Pos_Word(Target_Node) {
...
//# sourceMappingURL=c60aa3dab5a4b227c69a029ca66b1ade1e3961ea.js.map